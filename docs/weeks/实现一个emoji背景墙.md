---
updateTime: "2024-10-28 14:55:03"
desc: "实现一个emoji背景墙"
tags: "美化, 背景墙"
---
...
const useEmojiBackground = () => {
 ...
 // 性能优化：分批进行渲染
  const getRenderEmojis = () => {
    const height = emojiBgRef.value?.clientHeight || 0 //dom 高度 取 100vh 即视口高度
    const countLines = Math.floor(height / 40) //!SECTION: 计算行数
    const emojiPerLine = Math.floor(window.innerWidth / 40) * 2 //!SECTION: 计算每行 emoji 个数
    const interval = 5;//!SECTION: 渲染间隔

    //!SECTION: 递归渲染 emoji 数组 。5条5条进行渲染
    const renderChunk = (start:number,end:number) => {

      if(start >= end) return;
      if(start >= countLines) return;

      const chunk = new Array(Math.min(end,countLines) - start).fill(0).map(() => randomEmojis(emojiPerLine))//!SECTION: 5行emoji数组
