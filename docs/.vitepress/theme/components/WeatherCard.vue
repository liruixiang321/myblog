<template>
  <div
    class="weather flex min-h-[10em] min-w-[16em] flex-col items-center justify-center gap-[0.5em] rounded-[1.5em] bg-[#ecfeff] dark:bg-sky-900/80 px-[1em] py-[0.5em] font-nunito text-sky-400 shadow-lg"
  >
    <div class="flex h-fit w-full items-center justify-center gap-[1em]">
      ...
      <span class="h-[4em] w-[1px] rounded-full bg-sky-400"></span>
      <div class="flex flex-col items-start justify-center">
        <p class="text-[0.75rem] font-light">
          {{ weatherToday.city }}, {{ weatherToday.province }}
        </p>
        <p class="text-[1.5em] font-semibold">
    ...
    <div class="h-[0.5px] w-full rounded-full bg-sky-400"></div>
    <div class="flex items-center justify-between w-full h-fit">
      ...
      >
        ...
        >
          ...
          <span class="h-[0.5em] w-[1px] rounded-full bg-sky-400"></span>
          <p>Wind Speed</p>
          <span class="h-[0.5em] w-[1px] rounded-full bg-sky-400"></span>
          <p>{{ weatherToday.windpower }}</p>
        </div>
        <div
          class="flex flex-row items-center justify-center gap-[0.5em] p-[0.25em]"
        >
...
<script setup lang="ts">
  ...
  import useJsonp from "../utils/jsonp";
  const { error, fetchData } = useJsonp(
    "https://apis.map.qq.com/ws/location/v1/ip?key=QAIBZ-S2CKW-R4FRE-YPZOG-V3P3T-2LFUU"
  );
  let weatherToday = reactive({
    province: "",
    city: "",
    weather: "",
    temperature: "",
  ...
  function getWeather(res) {
    ...
    fetch(
      `https://restapi.amap.com/v3/weather/weatherInfo?city=${adcode}&key=37de2273cb2352249ad599b793b07b20`
    )
      .then((res) => res.json())
      .then((res) => {
        weatherToday.city = res.lives[0].city;
        weatherToday.province = res.lives[0].province;
        weatherToday.weather = res.lives[0].weather;
        weatherToday.temperature = res.lives[0].temperature;
        weatherToday.winddirection = res.lives[0].winddirection;
        weatherToday.windpower = res.lives[0].windpower;
        weatherToday.humidity = res.lives[0].humidity;
      })
      .catch(console.error);
  }
